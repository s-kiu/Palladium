...
export class BridgeDb {
  ...
  // ── permissions: read-only, and only until everyone has upgraded ───────────
  // The agent owns permissions now, in a file an operator can edit. What is
  // left here reads the old tables so a server upgrading from before that can
  // hand its groups and grants across once. When that hand-over is behind
  // everyone, these and their tables go.
