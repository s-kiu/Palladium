...
| `player.teleport` | `!teleport` | action | agent | experimental | 2.0.0 | `x` · number · required<br>`y` · number · required<br>`z` · number · required | Teleport an online player to world coordinates. Uses the engine's own placement call, then reads the position back — an ok carries where the player actually landed, and a teleport that did not move anyone fails rather than reporting success. |
