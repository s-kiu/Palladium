...
| `player.set_immortal` | `!set_immortal` | action | agent | experimental | 4.18.0 | `on` · bool · default true | Make a player unkillable, or mortal again. Sets the engine's IsImmortality flag and zeroes the rate at which enemies inflict damage on them — the flag holds on this build but the damage path does not consult it, so the rate is what does the work. Both are read back. |
| `player.set_frozen` | `!set_frozen` | action | agent | experimental | 4.20.0 | `on` · bool · default true | Hold a player still, or let them go. Pins the engine's own walk-speed multiplier to zero under a Palladium-owned flag name, so releasing it restores whatever the game had rather than a guess at normal. The multiplier is read back. |
| `player.set_flying` | `!set_flying` | action | agent | experimental | 4.20.0 | `on` · bool · default true | Start or end flight for a player, through the controller's own server call — the one a client sends when it takes off. Nothing reports flight back, so the result says what was asked for and marks itself unverified. |
