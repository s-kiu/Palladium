...
| `player.set_immortal` | `!set_immortal` | action | agent | experimental | 4.18.0 | `on` · bool · default true | Make a player unkillable, or mortal again. Sets Unreal's own bCanBeDamaged on the character, which is what TakeDamage consults — this build's IsImmortality and enemy damage-rate flags were both verified written and both left the player mortal. Read back, so a build that ignores the write says so. |
