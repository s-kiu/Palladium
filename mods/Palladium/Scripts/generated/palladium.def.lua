...
---@field set_immortal fun(target: string, params: PlayerSetImmortalParams, done?: fun(ok: boolean, err: string|nil, data: table)) # Make a player unkillable, or mortal again. Sets the engine's IsImmortality flag and zeroes the rate at which enemies inflict damage on them — the flag holds on this build but the damage path does not consult it, so the rate is what does the work. Both are read back.
...
---@field set_flying fun(target: string, params: PlayerSetFlyingParams, done?: fun(ok: boolean, err: string|nil, data: table)) # Start or end flight for a player, through the controller's own server call — the one a client sends when it takes off. Nothing reports flight back, so the result says what was asked for and marks itself unverified.
