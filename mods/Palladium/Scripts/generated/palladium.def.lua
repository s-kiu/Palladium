...
---@field set_immortal fun(target: string, params: PlayerSetImmortalParams, done?: fun(ok: boolean, err: string|nil, data: table)) # Make a player unkillable, or mortal again. Sets Unreal's own bCanBeDamaged on the character, which is what TakeDamage consults — this build's IsImmortality and enemy damage-rate flags were both verified written and both left the player mortal. Read back, so a build that ignores the write says so.
---@field set_frozen fun(target: string, params: PlayerSetFrozenParams, done?: fun(ok: boolean, err: string|nil, data: table)) # Hold a player still, or let them go. Pins the engine's own walk-speed multiplier to zero under a Palladium-owned flag name, so releasing it restores whatever the game had rather than a guess at normal. The multiplier is read back.
---@field set_flying fun(target: string, params: PlayerSetFlyingParams, done?: fun(ok: boolean, err: string|nil, data: table)) # Start or end flight for a player. Prefers ClientCheatFly, Unreal's own flight RPC, which is sent to the player's machine where flight is actually switched on; falls back to the ride take-off call. Nothing reports flight back, so the result names the call it used and marks itself unverified.
