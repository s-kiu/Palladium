...
---@field set_immortal fun(target: string, params: PlayerSetImmortalParams, done?: fun(ok: boolean, err: string|nil, data: table)) # Make a player unkillable, or mortal again. This build's own switches do not reach the client — bCanBeDamaged and IsImmortality were both verified set while the player went on dying — so the agent refills their health on its own tick instead. Enforcement rather than prevention: a hit big enough to kill between two ticks still kills.
