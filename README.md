...
- **Web panel** — sign in with the admin password at `http://<host>:3000`: connect addresses for your players, live server status, game version and one-click updates, online players with kick/ban/unban, loaded mods and LogicMods with enable/disable toggles, a grouped and searchable settings editor with diff-before-apply, backups with one-click create and rollback, a live server-log console, and an admin page with every vanilla server command. Sessions persist via cookie until you sign out. The panel needs **no docker.sock** — it drives the game through its REST API and the shared data volume.
